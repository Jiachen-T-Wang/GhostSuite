...
class DomainFileManager:
    ...
    def print_summary(self):
        ...
        for filename, (num_tokens, file_size) in all_files.items():
            print(
                f"  {filename}: {num_tokens:,} tokens ({file_size / 1024 / 1024:.1f} MB)"
            )


def main():
    parser = argparse.ArgumentParser(
        description="Tokenize Pile dataset by domain with streaming"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/scratch/gpfs/tw8948/pile_tokenized",
    ...
    try:
        ...
        if "promote_options" in str(e):
            print(
                "PyArrow version incompatibility detected. Using streaming dataset..."
            )
            dataset = load_dataset(dataset_name, data_files=data_files, streaming=True)
        else:
            raise e
...
def test_loading(output_dir="./tokenized_pile"):
    ...
    for filename in os.listdir(output_dir):
        if filename.endswith(".bin"):
            ...
            print(
                f"{filename}: {len(domain_data)} tokens, first 10: {domain_data[:10]}"
            )
