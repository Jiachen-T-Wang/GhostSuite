...
class CausalLMOutput:
    ...
    def __iter__(self):
        """
        For backward compatibility, we can use `logits, loss = model(inputs)`
        """
        yield self.logits
        yield self.loss
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        # Shape positional indices per-sample so embedding grads carry a batch dimension
        pos = torch.arange(0, t, dtype=torch.long, device=device).unsqueeze(0).expand(b, -1) # shape (b, t)
