...
Set the Pile data dirs (per-domain GPT-2 `.bin` files; see `examples/dvemb_lm/README.md`), then run
...
python examples/dvemb_lm/main.py --data_source pile --architecture GPT2-Small --device cuda \
...
python examples/dvemb_lm/experiments/plot_temporal_influence.py \
    --values examples/dvemb_lm/results/<run>/value/values.pt --out dvemb_temporal_influence_2026-06-30 \
