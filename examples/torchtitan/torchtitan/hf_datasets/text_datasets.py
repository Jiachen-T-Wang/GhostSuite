...
def _load_c4_500gb_dataset(dataset_path: str):
    """Load C4 500GB dataset with default configuration."""
    # Restrict to the text shards to avoid picking up metadata files such as stats.json
    data_files = {"train": os.path.join(dataset_path, "shard_*.jsonl")}
    return load_dataset("json", data_files=data_files, split="train", streaming=True)


def _load_pile_dataset(dataset_path: str):
    """Load The Pile test split stored as JSONL shards."""
    data_files = {"train": os.path.join(dataset_path, "shard_*.jsonl")}
    return load_dataset("json", data_files=data_files, split="train", streaming=True)


def _load_dclm_curated_dataset(dataset_path: str):
    """Load curated DCLM recipe shards."""
    data_files = {"train": os.path.join(dataset_path, "shard_*_processed.json*")}
    return load_dataset("json", data_files=data_files, split="train", streaming=True)
...
def _build_dclm_recipe_datasets() -> dict[str, DatasetConfig]:
    ...
    for idx in range(1, 38):
        recipe_dir = os.path.join(
            base_dir, f"Recipe_{idx}", "processed_data_curated", "processed_data"
        )
        config = DatasetConfig(
            path=recipe_dir,
            loader=_load_dclm_curated_dataset,
            sample_processor=_process_c4_text,
        )
        # Allow both zero-padded and non-padded dataset names.
        configs[f"recipe_{idx}"] = config
        configs[f"recipe_{idx:02d}"] = config
...
DATASETS.update(_build_dclm_recipe_datasets())
...
def build_text_validation_dataloader(
    ...
    seq_len = job_config.validation.seq_len or job_config.training.seq_len
