...
class Validation:
    ...
    """Sequence length for validation; defaults to training seq_len when None"""
    ...
    def __post_init__(self):
        ...
        assert (
            self.seq_len is None or self.seq_len > 0
        ), "validation seq_len must be positive"


@dataclass
class Ghost:
    enable: bool = False
    """Enable ghost GradDotProd integration."""

    val_batch_size: int = 4
    """Validation batch size appended to each training batch."""

    val_dataset: str = "pile_test"
    """Validation dataset name."""

    val_dataset_path: str | None = None
    """Optional path to validation dataset."""

    save_dir: str | None = None
    """Directory to save dot-product logs (defaults to job.dump_folder/ghost_dotprods)."""

    save_interval: int = 0
    """Interval (in steps) to persist dot-product logs; 0 disables autosave."""

    save_train_batch: bool = False
    """If True, store X_train/Y_train in dot-product logs; disable to reduce size."""

    log_grad_norms: bool = False
    """Record per-sample train gradient norms and aggregated validation grad norm."""

    use_dummy_bias: bool = True
    """Enable dummy bias trick for embeddings."""
