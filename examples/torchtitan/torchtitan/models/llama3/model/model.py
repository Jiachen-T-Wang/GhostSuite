...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if _DEBUG_ATTENTION_DTYPE:
            print(f"[model] Attention forward: x dtype: {x.dtype}")

        bs, seqlen, _ = x.shape
        xq, xk, xv = self.wq(x), self.wk(x), self.wv(x)

        # check xq, xk, xv's dtype
        if _DEBUG_ATTENTION_DTYPE:
            print(f"[model] Attention forward: xq dtype: {xq.dtype}, xk dtype: {xk.dtype}, xv dtype: {xv.dtype}")
