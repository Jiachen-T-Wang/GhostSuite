...
# output feeds the next linear): the validation gradient is then zeroed for every layer before the
# last, giving wrong dot products in plain MLPs. (Residual nets happen to be unaffected because the
# skip path carries the gradient.) Never enable GHOST_SUBTRACT_VAL=0 for training; add_hooks() warns
# once if it is set. Default subtract-val recovers the train grad post-backward as
# (total/train)*(grad - grad_val), reusing the grad_val already computed for the dot-product.
_SUBTRACT_VAL = os.getenv("GHOST_SUBTRACT_VAL", "1") == "1"
# Set once add_hooks() has warned about the known-incorrect masking path, so the warning fires once.
_MASKING_WARNED = False
...
def add_hooks(
    ...
    global _MASKING_WARNED
    if not _SUBTRACT_VAL and not _MASKING_WARNED:
        _MASKING_WARNED = True
        warnings.warn(
            "GHOST_SUBTRACT_VAL=0 selects the legacy masking path, which is retained only as the "
            "exact per-sample-gradient oracle for the equivalence tests. It is KNOWN-INCORRECT for "
            "training on residual-free models (e.g. plain MLPs) because it mutates shared saved "
            "activations. Unset GHOST_SUBTRACT_VAL (default subtract-val) for any real training.",
            RuntimeWarning,
            stacklevel=2,
        )
