...
def forward_swapper(module):
    ...
    if isinstance(module, (transformers.GPT2LMHeadModel, transformers.GPT2DoubleHeadsModel)):
        ...
        logger.warning(
            f"Model {module.__class__.__name__} is not supported by forward_swapper. "
            "You may or may not need to implement a custom forward function for it."
        )
...
def swap_gpt2_model_forward(model: Union[transformers.GPT2Model, transformers.GPT2DoubleHeadsModel]):
    ...
    def new_forward(
        ...
        output_hidden_states=None,
        return_dict=None,
        **kwargs, # For compatibility with newer versions of transformers
    ):
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
